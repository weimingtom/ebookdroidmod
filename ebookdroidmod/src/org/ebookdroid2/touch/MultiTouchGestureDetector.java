...
public class MultiTouchGestureDetector implements IGestureDetector {
	...
	private final static String TAG = "MultiTouchGestureDetector";
    ...
    public boolean onTouchEvent(final MotionEvent ev) {
        if ((ev.getAction() & MotionEvent.ACTION_POINTER_DOWN) == 
            ...
            if (D) {
                Log.e(TAG, "onTouchEvent(pointer down, " + ev.getPointerCount() + "): " + twoFingerPress + ", "
                        + twoFingerMove);
            }
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            if (D) {
            	Log.e(TAG, "onTouchEvent(move, " + ev.getPointerCount() + "): " + twoFingerPress + ", "
                        + twoFingerMove);
            }
        ...
        if ((ev.getAction() & MotionEvent.ACTION_POINTER_UP) == 
            ...
            if (D) {
            	Log.e(TAG, "onTouchEvent(pointer up, " + ev.getPointerCount() + "): " + twoFingerPress + ", "
                        + twoFingerMove);
            }
            if (twoFingerPress && ev.getPointerCount() == 2) {
                if (twoFingerMove) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_UP && multiEventCatched) {
            if (D) {
            	Log.e(TAG, "onTouchEvent(up, " + ev.getPointerCount() + "): " + twoFingerPress + ", "
                        + twoFingerMove);
            }
            if (twoFingerPress && ev.getPointerCount() < 2) {
                if (D) {
                	Log.e(TAG, "MultiTouchGestureDetector.onTouchEvent(): up pointer");
                }
                if (twoFingerMove) {
                    listener.onTwoFingerPinchEnd(calculateCenterEvent(ev));
                } else {
