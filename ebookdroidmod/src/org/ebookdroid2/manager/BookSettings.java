...
import org.ebookdroid2.manager.AppSettings.RotationType;
...
public class BookSettings implements CurrentPageListener {
    ...
    public int zoom = 100;
    public boolean splitPages; //全局
    public boolean splitRTL; //全局
    public BookRotationType rotation;
    public DocumentViewMode viewMode; //全局
    public PageAlign pageAlign = PageAlign.AUTO; //全局
    public PageAnimationType animationType = PageAnimationType.NONE; //全局
    public final List<Bookmark> bookmarks = new ArrayList<Bookmark>();
    public boolean cropPages; //全局
    public float offsetX;
    public float offsetY;
    public boolean nightMode; //全局
    public boolean positiveImagesInNightMode;//全局
    public int contrast = 100;//AppPreferences.CONTRAST.defValue; //全局
    public int exposure = 100;//AppPreferences.EXPOSURE.defValue; //全局
    public boolean autoLevels; //全局
    ...
    public BookSettings(final BookSettings current) {
        ...
        try {
            this.typeSpecific = current.typeSpecific != null ? 
            	new JSONObject(current.typeSpecific.toString()) : null;
    ...
    BookSettings(final JSONObject object) throws JSONException {
        ...
        this.rotation = BookRotationType.getByName(object.optString("rotation"), BookRotationType.UNSPECIFIED);
        this.viewMode = DocumentViewMode.getByName(object.optString("viewMode"), DocumentViewMode.VERTICALL_SCROLL);
        this.pageAlign = PageAlign.getByName(object.optString("pageAlign"), PageAlign.AUTO);
        this.animationType = PageAnimationType.getByName(object.optString("animationType"), PageAnimationType.NONE);
    ...
    public static enum BookRotationType {
        UNSPECIFIED("Unspecified", null),
        LANDSCAPE("Force landscape", RotationType.LANDSCAPE),
        PORTRAIT("Force portrait", RotationType.PORTRAIT);

        private final String resValue;
        private final RotationType orientation;

        private BookRotationType(final String resId, final RotationType orientation) {
            this.resValue = resId;
        ...
        public static BookRotationType getByName(String resValue, BookRotationType defValue) {
