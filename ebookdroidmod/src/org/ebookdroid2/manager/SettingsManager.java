...
public class SettingsManager {
    ...
    public static List<IBookSettingsChangeListener> listenerBookSettingsChange = new ArrayList<IBookSettingsChangeListener>();    
    private static BookSettingsUpdate updateThread;
    
    public static void addBookSettingsChangeListener(IBookSettingsChangeListener listener) {
    	if (listener != null) {
	    	listenerBookSettingsChange.remove(listener);
	    	listenerBookSettingsChange.add(listener);
    	}
    }
    public static void removeBookSettingsChangeListener(IBookSettingsChangeListener listener) {
    	if (listener != null) {
    		listenerBookSettingsChange.remove(listener);
    	}
    }
    ...
    public static BookSettings create(final long ownerId, final String fileName, final boolean temporaryBook,
        ...
        try {
            ...
            if (intent != null) {
            	current.persistent = Boolean.parseBoolean(safeString(intent.getStringExtra("persistent"), "true"));
                current.viewMode = DocumentViewMode.valueOf((safeString(intent.getStringExtra("viewMode"),current.viewMode.toString())));
                current.animationType = PageAnimationType.valueOf(safeString(intent.getStringExtra("animationType"), current.animationType.toString()));
                current.pageAlign = PageAlign.valueOf(safeString(intent.getStringExtra("pageAlign"), current.pageAlign.toString()));
                current.splitPages = Boolean.parseBoolean(safeString(intent.getStringExtra("splitPages"), Boolean.toString(current.splitPages)));
                current.cropPages = Boolean.parseBoolean(safeString(intent.getStringExtra("cropPages"), Boolean.toString(current.cropPages)));
                current.nightMode = Boolean.parseBoolean(safeString(intent.getStringExtra("nightMode"), Boolean.toString(current.nightMode)));
                if (intent.hasExtra("pageIndex")) {
                	final int pageIndex = Integer.parseInt(safeString(intent.getStringExtra("pageIndex"), Integer.toString(current.currentPage.viewIndex)));
                    current.currentPage = new PageIndex(current.splitPages ? current.currentPage.docIndex : pageIndex, pageIndex);
                    current.offsetX = Float.parseFloat(safeString(intent.getStringExtra("offsetX"), "0"));
                    current.offsetY = Float.parseFloat(safeString(intent.getStringExtra("offsetY"), "0"));
                }
            }
    ...
    private static BookSettings getBookSettingsImpl(final String fileName) {
        ...
        if (bs == null) {
            final String mpath = CacheManager.invertMountPrefix(fileName);
    ...
    public static void onBookSettingsActivityClosed(final BookSettings current) {
        ...
        try {
            ...
            //FIXME:代码删除了：AppSettings.fillBookSettings(current);
    ...
    public static void applyBookSettingsChanges(final BookSettings oldSettings, final BookSettings newSettings) {
        ...
        //事件响应
        if (listenerBookSettingsChange != null && listenerBookSettingsChange.size() > 0) {
        	for (IBookSettingsChangeListener listener : listenerBookSettingsChange) {
        		if (listener != null) {
        			listener.onBookSettingsChanged(oldSettings, newSettings, diff);
        		}
        	}
        }
    }
    
    //FIXME:这个类用于扫描变化，可能可以删除
    private static class BookSettingsUpdate extends Thread {
        ...
        public void run() {
            if (D) {
            	Log.e(TAG, "BookSettingsUpdate thread started");
            }
            while (run.waitFor(TimeUnit.MILLISECONDS, INACTIVITY_PERIOD)) {
                ...
                try {
                	...
                	th.printStackTrace();
                    if (D) {
                    	Log.e(TAG, "BookSettingsUpdate thread error: ", th);
                    }
                } finally {
                    lock.writeLock().unlock();
                }
                if (stored) {
                	//FIXME:代码删除了：onRecentBooksChanged();
                }
            }
            if (D) {
            	Log.e(TAG, "BookSettingsUpdate thread finished");
            }
        }
    }
