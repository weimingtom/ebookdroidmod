...
public class Bookmark implements Comparable<Bookmark> {
    ...
    public int compareTo(final Bookmark that) {
        ...
        int res = compareInt(this.page.docIndex, that.page.docIndex);
        if (res == 0) {
        	res = compareInt(this.page.viewIndex, that.page.viewIndex);
            if (res == 0) {
            	res = compareFloat(this.offsetY, that.offsetY);
                if (res == 0) {
                	res = compareFloat(this.offsetX, that.offsetX);
                }
            }
        }
        return res;
