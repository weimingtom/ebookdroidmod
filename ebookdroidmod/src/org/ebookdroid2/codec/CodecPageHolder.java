...
public class CodecPageHolder {
	...
	private final static String TAG = "CodecPageHolder";		
    ...
    protected <T> T access(long taskId, final Callable<T> operation, final boolean wait, final T defValue) {
        if (D) {
        	Log.e(TAG, ">>>CodecPageHolder#access");
        }
        final String name = operation.getClass().getSimpleName();
        while (true) {
            if (access.compareAndSet(false, true)) {
                if (D) {
                    Log.e(TAG, "Task " + taskId + ": " + name + ": operation started: " + pageIndex);
                }
                T result = null;
                try {
                    ...
                    if (D) {
                    	Log.e(TAG, "Task " + taskId + ": " + name + ": operation finished: " + pageIndex + " " + result);
                    }
                }
                return result;
            } else {
                if (!wait) {
                    if (D) {
                    	Log.e(TAG, "Task " + taskId + ": " + name + ": operation return with no waiting: " + pageIndex + " " + defValue);
                    }
                    return defValue;
                }
                synchronized (access) {
                    try {
                        if (D) {
                        	Log.e(TAG, "Task " + taskId + ": " + name + ": operation waiting for access: " + pageIndex);
                        }
                        access.wait(100);
    ...
    public final boolean lock() {
        ...
        if (D) {
        	Log.e(TAG, "Page locked: " + pageIndex + " " + res);
        }
        return res > 0;
    }
    ...
    public final boolean unlock() {
        ...
        if (D) {
        	Log.e(TAG, "Page unlocked: " + pageIndex + " " + res);
        }
        return res > 0;
    }
