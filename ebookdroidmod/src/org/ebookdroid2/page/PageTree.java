...
import org.ebookdroid2.event.IEvent;
...
public class PageTree {
	...
	private final static String TAG = "PageTree";
    ...
    public PageTree(final Page owner, Context context) {
        this.owner = owner;
        this.root = new PageTreeNode(owner, context);
        this.maxNodeId = 1;
    ...
    public boolean paintChildren(final EventDraw event, final PageTreeNode node, final RectF nodeRect) {
        ...
        if (childId < maxNodeId) {
            final PageTreeNode[] nodes = getNodes();
            for (final int end = Math.min(nodes.length, 
            		childId + PageTree.splitMasks.length); 
            	childId < end; childId++) {
    ...
    public boolean createChildren(final PageTreeNode parent) {
        ...
        for (int i = 0; i < splitMasks.length; i++, childId++) {
            if (nodes[childId] == null) {
                nodes[childId] = new PageTreeNode(owner, parent, 
                	childId, splitMasks[i], this.mContext);
            }
        }
    ...
    public boolean recycleParents(final PageTreeNode child, final List<Bitmaps> bitmapsToRecycle) {
        ...
        for (PageTreeNode p = getParent(childId, false); 
        		p != null; p = getParent(childId, false)) {
    ...
    public boolean isHiddenByChildren(final PageTreeNode parent, final ViewState viewState, final RectF pageBounds) {
        ...
        for (final int end = Math.min(nodes.length, 
            ...
            if (viewState.isNodeKeptInMemory(child, pageBounds) && 
            		!child.holder.hasBitmaps()) {
                return false;
            }
        }
        return true;
